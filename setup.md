...
- **Solution:**
  - In [`dotfiles-linux/waybar/.config/waybar/style.css`](file:///home/asim/setup/Dotfiles/dotfiles-linux/waybar/.config/waybar/style.css):
    ...
    - **Persistent Per-Animal Affection & 30-Minute Decay (`stats.json`):** Companion stats are persisted at runtime in [`dotfiles-linux/waybar/.config/waybar/scripts/island/stats.json`](file:///home/asim/setup/Dotfiles/dotfiles-linux/waybar/.config/waybar/scripts/island/stats.json) (ignored in git via `.gitignore`). Each companion animal (Neko, Usagi, Duck, Ghost, Crab, Inu) maintains its own independent `affection_score`, `last_pet_time`, and `last_decay_time`. Petting (Left-Click) increments affection for the currently active animal, while switching (Right-Click) instantly loads that companion's individual affection milestone, bond meter, and decay timer. Inactive animals decay by 1 point every 30 minutes independently (both online and offline across reboots).
